...
export const CardComponent: React.FC<CardComponentProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Top Box: Title and Range Badge */}
      <div
        className={`relative z-10 rounded-xl px-2 py-0.5 flex items-center justify-between gap-1 shadow-md backdrop-blur-md ${
          isBrown ? 'bg-amber-950/80 border border-amber-600/40' : 'bg-blue-950/80 border border-blue-500/40'
        }`}
      >
        <span className="font-black text-amber-200 text-[11px] sm:text-xs truncate leading-tight">
          {card.titleFa}
        </span>
        {card.range && (
          <span className="bg-amber-400 text-saloon-950 px-1 py-0.2 rounded text-[8px] sm:text-[9px] font-black leading-none shrink-0 shadow">
            برد {card.range}
          </span>
        )}
      </div>
        ...
        <div className="absolute inset-0 bg-black/88 backdrop-blur-md rounded-2xl p-2.5 flex flex-col justify-between text-right animate-in fade-in zoom-in-95 duration-150 z-30 border-2 border-amber-400">
          <div>
            <div className="flex items-center justify-between border-b border-white/15 pb-1 mb-1.5">
              <span className="font-black text-xs text-amber-300">{card.titleFa}</span>
